#!/bin/bash
# test_obj.sh — Run the test suite using direct object file generation (--obj)
#               instead of assembly+as+gcc.
#
# Usage:  ./test_obj.sh [compiler_path]
#   compiler_path  Path to the compiler binary.
...
# Expected results (same as test_stage1.sh)
declare -A EXPECTED
EXPECTED=(
    [01_return]=42
    [02_arithmetic]=7
...
for testfile in tests/*.c; do
    ...
    # Compile directly to .o
    if ! "$COMPILER" "$testfile" -c >/dev/null 2>&1; then
        if [ ! -f "$ofile" ]; then
    ...
    if [ ! -f "$ofile" ]; then
        echo "  FAIL  $name  (no .o generated, expected exit=$expected)"
        FAIL=$((FAIL + 1))
        continue
    fi

    # Link
    if ! gcc -no-pie -o "$binfile" "$ofile" 2>/dev/null; then
