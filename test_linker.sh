#!/bin/bash
# test_linker.sh — Run the test suite using the compiler's built-in linker
#                  (full pipeline: C -> binary .o -> internal ELF linker).
#
# This exercises the complete toolchain WITHOUT any external tools (no as, no gcc).
#
# Usage:  ./test_linker.sh [compiler_path]
...
# Expected exit codes (same as test_obj.sh / test_stage1.sh)
declare -A EXPECTED
EXPECTED=(
    [01_return]=42
    [02_arithmetic]=7
